...
func (e *Enterprise) passiveIncomeLoop() { // постоянная добыча +1 без условий
	...
	ticker := time.NewTicker(time.Second) // таймер, тикающий каждую секунду пока идет добыча
	defer ticker.Stop()
	...
	for {
		...
		case <-e.ctx.Done():
			return
		case <-ticker.C: // подожди, пока тикер отправит следующий тик, должна пройти 1 сек
		}
